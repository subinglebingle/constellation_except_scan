...
def make_sprites(n=100000, height=128, width=128): #h,w=64 였음 
    ...
    labels = np.zeros((n, 5))
    gt_masks = np.zeros((n, 8, height, width, 3), dtype=np.float16) ##ground truth를 위한 코드 #8=num_slots
    ...
    for i in range(n):
        ...
        y_pos = np.clip(((np.mean(y_positions)-0.2)/0.6)//0.2, 0, 4)  # 0~4 범위로 제한
        
        for k in range(0,len(sprites)): ##ground truth를 위한 코드
            gt_masks[i][k+1]=renderer.render([sprites[k]])
        
        foreground = np.sum(gt_masks[i][1:], axis=0)
        # 1.0(흰 화면)에서 foreground 마스크를 뺌, 값 범위는 0~1로 클리핑
        gt_masks[i][0] = np.clip(1.0 - foreground, 0, 1)
    ...
    return {'x_train': images[:8 * n // 10], #80%
            ...
            'gt_masks_train': gt_masks[:8 * n // 10], 
            'x_val': images[8 * n // 10: 9 * n // 10], #10%
            'labels_val': labels[8 * n // 10: 9 * n // 10],
            'gt_masks_val': gt_masks[8 * n // 10: 9 * n // 10],
            'x_test': images[9 * n // 10:], #10%
            'labels_test': labels[9 * n // 10:],
            'gt_masks_test': gt_masks[9 * n // 10:]
            }
...
class Sprites(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        return img, self.labels[idx], masks